...
public class SchoolRepository extends BaseRepository<School> {
	...
	@Override
	protected PreparedStatement fillInsertStatement(PreparedStatement statement, School row) {
		try {
	...
	@Override
	protected PreparedStatement fillUpdateStatement(PreparedStatement statement, School row) {
		try {
			statement.setString(1, row.getName());
	...
	protected String createUpdateStatement() {
		return "UPDATE " + tableName + " set " + COLUMN_NAME + " = ?, " + COLUMN_SEMESTER + " = ?";
	}
}
