...
import ch.noebuerki.gradelyapi.database.objects.Grade;
...
public class GradeRepository extends BaseRepository<Grade> {
	...
	@Override
	protected PreparedStatement fillStatement(PreparedStatement statement, Grade row) {
		try {
	...
	protected String createUpdateStatement() {
		return "UPDATE " + tableName + " set " + COLUMN_USER_ID + " = ?, " + COLUMN_SEMESTER_ID + " = ?, " + COLUMN_NAME + " = ?, " + COLUMN_VALUE + " = ?, " + COLUMN_WEIGHT + " = ?";
	}
}
